...
public class GT_GUIContainer_Research extends GuiContainer implements INEIGuiHandler {
    ...
    int researchState =0;
    int mResearchProgress;

    public  GT_GUIContainer_Research(GT_Recipe_ResearchStation aRecipe, EntityPlayer aPlayer, IGregTechTileEntity aTerminal, int aResearchState, IInventory aCopyFrom,int aResearchProgress){
        super(new GT_Container_Research(aPlayer.inventory,aRecipe.mDataOrb,152,-20));
        mRecipe = aRecipe;
        mPlayer = aPlayer;
        terminal = aTerminal;
        mBackgrpund =  new ResourceLocation(GT_Values.RES_PATH_GUI+"basicmachines/researches/ResearchPage.png");
        mDesription = aRecipe.mRecipePageText;
        researchState = aResearchState;
        mCopyFrom = aCopyFrom;
        mResearchProgress = aResearchProgress;
    }
    ...
    public void drawStartButton(int mx, int my){
        if(researchState!=1&&researchState!=2)
            return;
        ...
        if(isMouseOverRect(mx,my,buttonX,buttonY,80,35)){
            ...
            if(Mouse.isButtonDown(0)){
                //startResearch
                //Minecraft.getMinecraft().displayGuiScreen(new GT_GUIContainer_ChooseStation(mRecipe,mPlayer,terminal));
                if(!alreadySent&&researchState == 1) {
                    boolean b = terminal.isServerSide();
                ...
                else if(researchState == 2&&mCopyFrom!=null){
